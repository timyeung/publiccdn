window.alipanArtPlugins = window.alipanArtPlugins || function(t) {
    var e = {
        version: "1.0.2",
        init: t => Promise.all([e.readyHls(), e.readyArtplayer(), e.readyM3u8Parser()]).then((() => e.initArtplayer(t))),
        readyHls: function() {
            return window.Hls || unsafeWindow.Hls ? Promise.resolve() : e.loadJs("https://unpkg.com/hls.js/dist/hls.min.js")
        },
        readyArtplayer: function() {
            return window.Artplayer || unsafeWindow.Artplayer ? Promise.resolve() : e.loadJs("https://unpkg.com/artplayer/dist/artplayer.js")
        },
        readyM3u8Parser: function() {
            return window.m3u8Parser || unsafeWindow.m3u8Parser ? Promise.resolve() : e.loadJs("https://unpkg.com/m3u8-parser/dist/m3u8-parser.min.js")
        },
        initArtplayer: function(n) {
            if (n = e.initOption(n), !Array.isArray(n.quality) || !n.quality.find((t => t?.url))) return alert("获取播放信息失败，请刷新网页重试"), Promise.reject("No available playUrl");
            const s = window.Artplayer || unsafeWindow.Artplayer;
            Object.assign(s, {
                PLAYBACK_RATE: [.5, .75, 1, 1.25, 1.5, 2, 3, 4, 5],
                ASPECT_RATIO: ["default", "4:3", "16:9", "自动拉伸"]
            });
            const i = new s(n, (e => {
                t.length % 8 !== t.length && t.forEach((t => {
                    e.plugins.add(t())
                }))
            }));
        ...
        initOption: function(t) {
            const e = {
                    ...
                    contextmenu: [{
                        html: "检查更新",
                        click: function(t, e) {
                            window.open("https://scriptcat.org/zh-CN/script-show-page/162", "_blank"), t.show = !1
                        }
                    }, {
                        html: "加油作者",
                        click: function(t, e) {
                    ...
                    customType: {
                        hls: function(t, e, n) {
                            ...
                            if (s.isSupported()) {
                                ...
                                i.loadSource(e), i.attachMedia(t), n.hls = i, n.on("destroy", (() => i.destroy()))
                            } else t.canPlayType("application/vnd.apple.mpegurl") ? t.src = e : n.notice.show = "Unsupported playback format: m3u8"
                        }
                    },
                    flip: !0,
                    icons: {
            ...
            } = n?.video_preview_play_info || {}, d = u || l;
            if (!Array.isArray(d) || !d.length) return e; {
                ...
                d.forEach(((e, n) => {
                    Object.assign(e, {
                        html: t[e.template_id] + (e.description ? "（三方VIP）" : e.url ? "" : "（VIP）"),
                        type: "hls"
                    })
                })), d.sort(((t, e) => t.template_height - e.template_height)), d.findLast((t => t.url)).default = !0, Object.assign(e, {
                    quality: d
                })
            }
            const {
            ...
            if (Array.isArray(m) && m.length) {
                ...
                })), (m.find((t => ["chi"].includes(t.language))) || m.find((t => t.url)) || {}).default = !0, Object.assign(e, {
                    subtitlelist: m
                })
            }
            const g = i || [];
            return Array.isArray(g) && g.length && ((g.find((t => t.file_id === o)) || {}).default = !0, Object.assign(e, {
                playlist: g
            })), e
        },
        loadJs: function(t) {
            return window.instances || (window.instances = {}), window.instances[t] || (window.instances[t] = new Promise(((e, n) => {
                const s = document.createElement("script");
                s.src = t, s.type = "text/javascript", s.onload = e, s.onerror = n, Node.prototype.appendChild.call(document.head, s)
            }))), window.instances[t]
        }
    };
    return console.info(`%c alipanArtPlugins %c ${e.version} %c https://scriptcat.org/zh-CN/users/13895`, "color: #fff; background: #5f5f5f", "color: #fff; background: #4bc729", ""), e
}([function() {
    return t => {
        ...
        var a = Date.now(),
            o = 0;
        return n.on(e.Events.ERROR, ((r, l) => {
            if (l.fatal) switch (i.show = `当前带宽: ${Math.round(n.bandwidthEstimate/1024/1024/8*100)/100} MB/s`, l.type) {
                case e.ErrorTypes.NETWORK_ERROR:
                    l.details === e.ErrorDetails.MANIFEST_LOAD_ERROR || l.details === e.ErrorDetails.MANIFEST_LOAD_TIMEOUT || l.details === e.ErrorDetails.MANIFEST_PARSING_ERROR ? n.loadSource(n.url) : l.details === e.ErrorDetails.FRAG_LOAD_ERROR ? ++o < 10 && (n.loadSource(n.url), n.media.currentTime = t.currentTime, n.media.play()) : n.startLoad();
                    break;
            ...
            } else if (l.type === e.ErrorTypes.NETWORK_ERROR) l.details === e.ErrorDetails.FRAG_LOAD_ERROR && function(t) {
                var e = arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : 6e3,
                    n = t.match(/&x-oss-expires=(\d+)&/);
                return n ? +"".concat(n[1], "000") - e < Date.now() : Date.now() - a > 3e5 - e
            }(n.url) && (o = 0, a = Date.now(), n.stopLoad(), t.emit("reload-start", s.quality))
        })), t.on("reload-can", (e => {
            ! function(e) {
                s.quality = e;
                const a = n.url = (e.find((t => t.default)) || e.findLast((t => t.url)) || {}).url;
                fetch(a).then((t => t.ok ? t.text() : Promise.reject())).then((e => {
                    const s = new(window.m3u8Parser || unsafeWindow.m3u8Parser).Parser;
                    s.push(e), s.end();
    ...
    return function(t) {
        ...
        function a() {
            const a = n.quality.find((t => t.default)) || n.quality.findLast((t => t.url));
            e.update({
                name: "quality",
                html: a ? a.html : "",
                selector: n.quality,
                onSelect: function(e) {
                    e.url ? (t.switchQuality(e.url), s.show = `${i.get("Switch Video")}: ${e.html}`) : s.show = e.description || "视频地址不可用"
                }
            })
        }
        return a(), t.on("playlist-switch-can", (() => {
            setTimeout(a, 1e3)
        })), t.on("reload-can", (() => {
            setTimeout(a, 1e3)
        })), {
    ...
    return function(t) {
        ...
        function a(n) {
            e.playlist[n] ? "function" == typeof s.onchanged && s.onchanged(e.playlist[n]) : n >= e.playlist.length && (t.notice.show = "没有下一集了")
        }
        return t.controls.add({
            ...
            onSelect: t => (a(i = t.index), s.showtext ? "播放列表" : s.icon)
            ...
            const {
            ...
            } = s?.video_preview_play_info || {}, p = function(t) {
                if (Array.isArray(t) && t.length) {
                    ...
                    return t.forEach(((t, n) => {
                        Object.assign(t, {
                            html: e[t.template_id] + (t.description ? "（三方VIP）" : t.url ? "" : "（VIP）"),
                            type: "hls"
                        })
                    })), t.sort(((t, e) => t.template_height - e.template_height)), t.findLast((t => t.url)).default = !0, t
                }
                return []
            }(c || r);
            if (!p.length) return alert("获取播放信息失败，无法切换视频"); {
                ...
                Object.assign(e, {
                    ...
                    (document.querySelector("[class^=header-file-name], [class^=header-center] div span") || {}).innerText = a, n.show = `切换视频: ${a}`
                })).catch((() => {
                    n.show = `视频地址不可用: ${a}`
                }))
            }
            e.subtitlelist = function(t) {
                if (Array.isArray(t) && t.length) {
                    ...
                    return t.forEach((function(t, n) {
                        Object.assign(t, {
                            html: (e[t.language] || t.language || "未知语言") + "（vtt）",
                            name: "内置字幕",
                            type: "vtt"
                        })
                    })), (t.find((t => ["chi"].includes(t.language))) || t[0] || {}).default = !0, t
                }
                return []
            }(o)
        })), t.on("video:ended", (() => {
    ...
    return function(t) {
        ...
        return t.setting.add({
            ...
            switch: !!e,
            onSwitch: function(e) {
                return e.tooltip = e.switch ? "关闭" : "开启", t.storage.set("auto-next", !e.switch), t.notice.show = "自动连续播放：" + (e.switch ? "关闭" : "开启"), !e.switch
            }
        }), {
            name: "autonext"
        }
    ...
    return t => {
        ...
        function l(t = []) {
            ...
            Object.assign(i.subtitle, {
                ...
                onSelect: function(t, e) {
                    ...
                    return Object.assign(i.subtitle, {
                        url: s,
                        type: a
                    }), n.switch(t.url, t), t.html
                }
            })
        }
        ...
        function c(t) {
            return function(t) {
                return new Promise(((e, n) => {
                    ...
                    s.readAsText(t, "UTF-8"), s.onload = function(n) {
                        var i = s.result;
                        return i.indexOf("�") > -1 && !s.markGBK ? (s.markGBK = !0, s.readAsText(t, "GBK")) : i.indexOf("") > -1 && !s.markBIG5 ? (s.markBIG5 = !0, s.readAsText(t, "BIG5")) : void e(i)
        ...
        function u(t, e) {
            return e instanceof Element ? Node.prototype.appendChild.call(t, e) : t.insertAdjacentHTML("beforeend", String(e)), t.lastElementChild || t.lastChild
        }
        return s.$subtitleLocalFile = u(s.$container, '<input class="subtitleLocalFile" type="file" accept="webvtt,.vtt,.srt,.ssa,.ass" style="display: none;">'), t.setting.add({
            width: 220,
            html: "字幕设置",
            ...
            selector: [{
                ...
                switch: !0,
                onSwitch: function(e) {
                    return e.tooltip = e.switch ? "隐藏" : "显示", t.subtitle.show = !e.switch, !e.switch
                }
            }, {
                html: "字幕偏移",
                ...
                onSelect: function(t, e, i) {
                    switch (t.name) {
                        case "color-presets":
                            "INPUT" === i.target.nodeName && n.style({
                        ...
                        case "color-default":
                            ...
                            break;
                        case "color-picker":
                            s.$colorPicker || (s.$colorPicker = u(s.$player, '<input hidden type="color">'), s.$colorPicker.oninput = t => {
                                n.style({
                                    color: t.target.value
                                })
                            }), s.$colorPicker.click()
                    }
                    return '<label style="display: flex;"><span style="width: 18px;height: 18px;display: inline-block;border-radius: 50%;box-sizing: border-box;cursor: pointer;background: ' + s.$subtitle.style.color + ';"></span></label>'
                }
            }, {
                html: "字体类型",
                ...
                onSelect: function(t, e, n) {
                    var a;
                    return "file" === t.name && (a = s.$subtitleLocalFile, a.click(), new Promise((function(t, e) {
                        a.onchange = e => {
                        ...
                        i.subtitlelist = (i.subtitlelist || []).concat([t]), l(i.subtitlelist)
                    })), !1
                }
            }]
        }), (i.subtitlelist || []).length && l(i.subtitlelist), t.on("restart", (() => {
            (i.subtitlelist || []).length ? l(i.subtitlelist) : e["subtitle-list"] && e.remove("subtitle-list")
        })), t.on("subtitlelist-add", (t => {
            i.subtitlelist = (i.subtitlelist || []).concat(t), (i.subtitlelist || []).length && l(i.subtitlelist)
        })), {
            name: "subtitle"
        }
    }
    ...
    return t => {
        function e() {
            t.libass && t.subtitle.show && (t.template.$subtitle.style.display = "none", (t.libass.canvasParent || t.libass._canvasParent).style.display = "block", t.libass.resize())
        }

        function n() {
            t.libass && (t.template.$subtitle.style.display = t.subtitle.show ? "block" : "none", (t.libass.canvasParent || t.libass._canvasParent).style.display = "none")
        }
        ...
        function s(e) {
            ...
            return function(t) {
                ...
                window.instances[t] || (window.instances[t] = new Promise(((e, n) => {
                    const s = document.createElement("script");
                    s.src = t, s.type = "text/javascript", s.onload = e, s.onerror = n, Node.prototype.appendChild.call(document.head, s)
                })));
                return window.instances[t]
            }(n).then((() => (Object.assign(e, {
                ...
                return (t.libass.canvasParent || t.libass._canvasParent).style.cssText = "position: absolute;top: 0;left: 0;width: 100%;height: 100%;user-select: none;pointer-events: none;z-index: 20;", t.libass
            })))))
        }
        return t.on("subtitle", (t => {
            t ? e() : n()
            ...
            return function() {
                ...
                return function(t) {
                    if (unsafeWindow.queryLocalFonts) {
                        const e = {};
                        return t && (e.postscriptNames = Array.isArray(t) ? t : [t]), unsafeWindow.queryLocalFonts(e).then((t => t && t.length ? t : Promise.reject()))
                    }
                    return console.warn("Not Local fonts API"), Promise.reject()
                }().then((t => {
                    const n = t.filter((t => t.fullName.match(/[\u4e00-\u9fa5]/))),
                        i = n.find((t => ["微软雅黑"].some((e => t?.fullName === e))))?.fullName || n.sort((() => .5 - Math.random()))[0]?.fullName;
                ...
                !s || "ass" !== i && "ssa" !== i ? (n(), t.libass.freeTrack()) : (! function(e) {
                    t.libass && e && (t.libass.freeTrack(), t.libass.setTrackByUrl(e))
                }(s), e())
            })).catch((t => {
            ...
            t.libass && (t.libass.destroy && t.libass.destroy(), t.libass.dispose && t.libass.dispose(), t.libass = null)
        })), {
            name: "libass"
        }
    }
    ...
    return t => {
        ...
        return n.add({
            ...
            selector: [{
                ...
                switch: !!a,
                onSwitch: function(e) {
                    return e.tooltip = e.switch ? "关闭" : "开启", t.storage.set("sound-enhancer", !e.switch), t.joySound && t.joySound.setEnabled(!e.switch), i.show = "音质增强：" + (e.switch ? "关闭" : "开启"), !e.switch
                }
            }, {
                html: "音量增强",
                ...
                onRange: function(e) {
                    const n = e.range / 10;
                    return t.joySound && t.joySound.setVolume(n), i.show = "音量增强：" + (100 + 100 * n) + "%", e.range + "x"
                }
            }]
        }), t.once("video:playing", (function() {
            const s = window.Joysound || unsafeWindow.Joysound;
            s && s.isSupport() && (t.joySound = t.joySound || new s, t.joySound.hasSource() || t.joySound.init(e.$video), t.storage.get("sound-enhancer") && (t.joySound.setEnabled(!0), n.update({
                name: "sound-enhancer",
                html: "音质增强",
                switch: !0
            })))
    ...
    return t => {
                    ...
                    if (0 == s) document.body.removeChild(t);
                        ...
                        if (n)
                            if (n.match(/^202[\d]{22,25}$/)) {
                                ...
                                e.getItem("users").then((t => {
                                    var s;
                                    t && t.ON == n || (s = n, a().then((t => (0 === Date.parse(t.expire_time) || e.setItem("users", Object.assign(t || {}, {
                                        expire_time: new Date(Date.now() + 864e3).toISOString()
                                    })).then((t => {
                                        e.setItem("users_sign", btoa(encodeURIComponent(JSON.stringify(t)))), GM_setValue("users_sign", btoa(encodeURIComponent(JSON.stringify(t))))
                                    })), function(t, e) {
                                        return delete t.createdAt, delete t.updatedAt, delete t.objectId, o({
                                            url: "https://sxxf4ffo.lc-cn-n1-shared.com/1.1/classes/aliyundrive",
                                            data: JSON.stringify(Object.assign(t, {
                                                ON: e
                                            }))
                                        })
                                    }(t, s))))).catch((() => {
        ...
        function i() {
            return e.getItem("users").then((t => t?.expire_time ? e.getItem("users_sign").then((n => Math.max(Date.parse(t.expire_time) - Date.now(), 0) && n === btoa(encodeURIComponent(JSON.stringify(t))) && GM_getValue("users_sign") === btoa(encodeURIComponent(JSON.stringify(t))) ? t : a().then((t => Math.max(Date.parse(t?.expire_time) - Date.now(), 0) ? e.setItem("users", t).then((t => (e.setItem("users_sign", btoa(encodeURIComponent(JSON.stringify(t)))), GM_setValue("users_sign", btoa(encodeURIComponent(JSON.stringify(t)))), t))) : (e.removeItem("users"), e.removeItem("users_sign"), GM_deleteValue("users_sign"), Promise.reject()))))) : GM_getValue("users_sign") ? e.setItem("users", {
                expire_time: (new Date).toISOString()
            }).then((() => i())) : (GM_setValue("users_sign", 0), Promise.reject())))
        }
        ...
        return t.once("video:playing", (function() {
            // const e = new Date;
            // n = setInterval((() => {
            //     const n = new Date - e;
            //     Math.floor(n / 1e3 / 60 % 60) % 9 || !t.playing || i().catch((e => {
            //         t.pause(), s()
            //     }))
            // }), 6e3)
        })), t.once("destroy", (function() {
